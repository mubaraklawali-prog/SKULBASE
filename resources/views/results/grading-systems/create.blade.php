            ...
            <form method="POST" action="{{ route('results.grading-systems.store') }}">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="grade" value="{{ old('grade') }}" required placeholder="e.g. A+, A, B+, B, C, D, F" style="width: 100%; padding: 10px 16px; border-radius: 8px; border: 1px solid #dee2e6; font-size: 14px;">
                    </div>
                    <div class="col-md-6 mb-3">
                        <label style="display: block; font-size: 13px; font-weight: 600; color: #6c757d; margin-bottom: 6px;">Minimum Score (%) *</label>
                        <input type="number" name="min_score" value="{{ old('min_score') }}" required min="0" max="100" step="0.01" placeholder="0" style="width: 100%; padding: 10px 16px; border-radius: 8px; border: 1px solid #dee2e6; font-size: 14px;">
                    </div>
                    <div class="col-md-6 mb-3">
                        <label style="display: block; font-size: 13px; font-weight: 600; color: #6c757d; margin-bottom: 6px;">Maximum Score (%) *</label>
                        <input type="number" name="max_score" value="{{ old('max_score') }}" required min="0" max="100" step="0.01" placeholder="100" style="width: 100%; padding: 10px 16px; border-radius: 8px; border: 1px solid #dee2e6; font-size: 14px;">
                    </div>
                    <div class="col-md-6 mb-3">
                        <label style="display: block; font-size: 13px; font-weight: 600; color: #6c757d; margin-bottom: 6px;">Grade Point</label>
                        <input type="number" name="grade_point" value="{{ old('grade_point') }}" min="0" step="0.01" placeholder="e.g. 4.0" style="width: 100%; padding: 10px 16px; border-radius: 8px; border: 1px solid #dee2e6; font-size: 14px;">
                    </div>
                    <div class="col-md-6 mb-3">
                        <label style="display: block; font-size: 13px; font-weight: 600; color: #6c757d; margin-bottom: 6px;">Remark *</label>
                        <input type="text" name="remark" value="{{ old('remark') }}" required placeholder="e.g. Excellent, Very Good, Good" style="width: 100%; padding: 10px 16px; border-radius: 8px; border: 1px solid #dee2e6; font-size: 14px;">
                    </div>
                </div>
                <div class="d-flex justify-content-end gap-2">
                    ...
                    <button type="submit" class="btn" style="background: #4f9cf7; color: #fff; border-radius: 8px; padding: 10px 24px; font-weight: 500; border: none; cursor: pointer;">Create Rule</button>
                </div>
            </form>
        </div>
    </div>
