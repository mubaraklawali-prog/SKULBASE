...
<div class="welcome-section">
    ...
    @if($rankings->isNotEmpty())
        <div class="card stat-card">
            <div class="card-body">
                <h5 style="font-weight: 600; margin-bottom: 16px; color: #1a1a2e;">{{ $exam->name ?? '' }} — {{ $class->name ?? '' }} Rankings</h5>
                    ...
                    <table class="table table-hover mb-0">
                        <thead>
                            <tr style="background: #f8f9fa;">
                                ...
                                <th style="padding: 14px 20px; font-size: 13px; font-weight: 600; color: #6c757d; text-transform: uppercase; letter-spacing: 0.5px;">Subjects Passed</th>
                                <th style="padding: 14px 20px; font-size: 13px; font-weight: 600; color: #6c757d; text-transform: uppercase; letter-spacing: 0.5px;">Attendance</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($rankings as $card)
                                <tr>
                                    <td style="padding: 14px 20px; font-weight: 700; font-size: 18px; color: {{ $card->class_position <= 3 ? '#1a1a2e' : '#6c757d' }};">{{ $card->class_position ? $this->ordinal($card->class_position) : '—' }}</td>
                                    <td style="padding: 14px 20px; font-weight: 500;">
                                        <a href="{{ route('results.computations.show', $card) }}" style="color: #333; text-decoration: none;">{{ $card->student->full_name ?? '—' }}</a>
                                    </td>
                                    <td style="padding: 14px 20px; font-weight: 600;">{{ number_format($card->average_score, 1) }}%</td>
                                    ...
                                    <td style="padding: 14px 20px; color: #6c757d;">{{ $card->overall_remark ?? '—' }}</td>
                                    <td style="padding: 14px 20px; color: #0f5132; font-weight: 600;">{{ $card->subjects_passed }}/{{ $card->total_subjects }}</td>
                                    <td style="padding: 14px 20px; color: #6c757d;">{{ $card->attendance_percentage ? number_format($card->attendance_percentage, 1) . '%' : '—' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
    ...
    @elseif($selectedExam && $selectedClass)
        <div class="card stat-card">
            <div class="card-body" style="padding: 40px; text-align: center;">
                <p class="text-muted" style="margin: 0; font-size: 15px;">No report cards found for this exam and class.</p>
            </div>
        </div>
    @else
        <div class="card stat-card">
            <div class="card-body" style="padding: 60px; text-align: center;">
                <p class="text-muted" style="margin: 0; font-size: 15px;">Please select an exam and class above to view rankings.</p>
            </div>
        </div>
    @endif
</div>
