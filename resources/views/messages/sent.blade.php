...
<div class="welcome-section">
    ...
    <div class="d-flex gap-2 mb-4">
        <a href="{{ route('messages.inbox') }}" class="sb-btn sb-btn-secondary">Inbox</a>
        <a href="{{ route('messages.sent') }}" class="sb-btn sb-btn-primary">Sent</a>
    </div>
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('messages.sent') }}" class="row g-2 align-items-end">
                ...
                @if(request()->has('search'))
                    <div class="col-md-1">
                        <a href="{{ route('messages.sent') }}" class="sb-btn sb-btn-secondary w-100 text-center">Clear</a>
                    </div>
                @endif
            </form>
        </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($message->status === 'unread')
                                        <span class="sb-badge sb-badge-pending">Unread</span>
                                    @else
                                        <span class="sb-badge sb-badge-active">Read</span>
                                    @endif
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" style="text-align: center; padding: 40px 20px; color: #6c757d;">
                                    No sent messages. <a href="{{ route('messages.create') }}" style="color: var(--primary);">Send your first message</a>.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
