...
@section('title', "My Child's Results - Skulbase")
    ...
    @if($children->count() > 1)
        <div class="card stat-card mb-4">
            <div class="card-body">
                ...
                <form method="GET" action="{{ route('parent.results.index') }}" class="child-selector" id="childSelectorForm">
                    <div class="row g-2">
        ...
        <div class="card stat-card mb-4">
            <div class="card-body py-3 px-4">
                <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                    ...
                    <div>
                        <form method="GET" action="{{ route('parent.results.index') }}" class="d-inline-flex align-items-center gap-2">
                                        ...
                                        @foreach($grouped as $examName => $examResults)
                                            ...
                                            @foreach($examResults as $result)
                                                <tr>
                                                    ...
                                                    <td style="padding: 12px 16px;">{{ $result->assessmentType->name ?? $result->assessment_type ?? '—' }}</td>
                                    ...
                                    <div class="report-card-item">
                                        <div class="d-flex justify-content-between align-items-start mb-2">
                                            <div>
                                                ...
                                                <small class="text-muted">{{ $selectedStudent->full_name }}</small>
                                            </div>
                                            <span class="grade-badge status-active">{{ $card->grade ?? '—' }}</span>
                                        </div>
                                        <div class="d-flex justify-content-between align-items-center mt-3">
                                            <div>
                                                <div style="font-size: 12px; color: #6c757d; margin-bottom: 2px;">Average</div>
                                                <div style="font-size: 22px; font-weight: 700; color: #0a1628;">{{ $card->average ?? '—' }}<span style="font-size: 13px; color: #6c757d; font-weight: 400;">%</span></div>
                                            </div>
                                            <a href="{{ route('parent.results.report-card', ['reportCard' => $card->id]) }}"
