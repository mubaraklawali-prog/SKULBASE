...
@section('title', 'Subject Performance - Skulbase')
...
<div class="welcome-section">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted mb-0">Subject-wise analysis</p>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('reports.export.subject-performance.csv', request()->query()) }}" class="btn" style="background: #198754; color: #fff; border-radius: 8px; padding: 10px 20px; font-weight: 500; text-decoration: none;">Export CSV</a>
            <a href="{{ route('reports.export.subject-performance.pdf', request()->query()) }}" class="btn" style="background: #dc3545; color: #fff; border-radius: 8px; padding: 10px 20px; font-weight: 500; text-decoration: none;">Export PDF</a>
    ...
    <form method="GET" action="{{ route('reports.academic.subject-performance') }}" class="card stat-card mb-4">
                    ...
                    <table class="table table-hover mb-0">
                        <thead>
                            <tr style="background: #f8f9fa;">
                                ...
                                <th style="padding: 14px 20px; font-size: 13px; font-weight: 600; color: #6c757d; text-transform: uppercase; letter-spacing: 0.5px;">Performance</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($performance as $index => $item)
                                <tr>
                                    ...
                                    <td style="padding: 14px 20px; font-weight: 500;">{{ $item['subject']->name }}</td>
                                    <td style="padding: 14px 20px;">{{ $item['students'] }}</td>
    ...
    @else
        <div class="card stat-card">
            <div class="card-body" style="padding: 60px; text-align: center;">
                <p class="text-muted" style="margin: 0; font-size: 15px;">No subject performance data available. Please select an exam.</p>
            </div>
        </div>
    @endif
</div>
